<div>
    <x-mary-header subtitle="Here you can add or edit the details of post." >

        <x-slot:title class="text-4xl">
            {{ checkData($post_id)?'Edit':'New' }} Post
        </x-slot:title>
    ...
    <form wire:submit.prevent="{{checkData($post_id)?'Save':'Submit'}}">
        <div class="grid grid-cols-12 gap-3">
            <div class="col-span-12 lg:col-span-9">
                <x-mary-card class="shadow border mb-5">
                    <x-mary-input wire:model.live="request.title"
                    ...
                    <!--end::Row-->
                    <x-mary-textarea wire:model="request.desc"
                    ...
                    <div x-data="{open:false}">
                        ...
                        <div>
                            <div>
                                ...
                                <a href="{{ BackendHelper::getSlugPrefix() }}{{ $request['slug'] ??'' }}"
                                   class="text-sm"
                                   target="_blank"
                                   style="color: #006621"
                                >{{ BackendHelper::getSlugPrefix() }}{{ $request['slug'] ??'' }}</a>
                                <p>
            ...
            <div class="col-span-12 lg:col-span-3 relative">
                <div class="sticky top-0 left-0">
                    <x-mary-card class="shadow border">
                        <div class="mb-5">
                            <x-mary-button label="Save Post"
                                           class="btn-primary"
                                           spinner="{{checkData($post_id)?'Save':'Submit'}}"
                                           type="submit"
                            />
                        </div>
                        <div class="flex mb-5">
                        ...
                        <x-mary-datepicker :config="['altFormat' => 'm/d/Y']"
                        ...
                        />
                        <div class="mb-5">
                            <x-mary-choices wire:model="request.category"
                                            label="Primary Category"
                                            :options="\App\Models\BlogCategory::orderBy('position')->get()"
                                            option-label="name"
                                            option-value="id"
                                            single
                            />
                        ...
                        <div class="mb-5">
                            <x-mary-choices wire:model="postCategoryRequest"
                                            label="Secondary Category"
                                            :options="\App\Models\BlogCategory::getInOrder($postCategoryRequest)"
                                            option-label="name"
                                            option-value="id"
                                            multiple
                            />
                        ...
                        <div class="mb-5">
                            <x-mary-select wire:model.live="request.read_time_format"
                                           label="Post Read Time Format"
                                           :options="[ ['label'=>'Auto','value'=>0],['label'=>'Manual','value'=>1] ]"
                                           option-value="value"
                                           option-label="label"
                            />
                        </div>
                        <div class="{{ $request['read_time_format']?'':'hidden' }} mb-5" >
                            <x-mary-input wire:model="request.read_time"
                                          label="Post Read Time (Min)"
                                          type="number"
                                          min="0"
                            />
                        </div>
